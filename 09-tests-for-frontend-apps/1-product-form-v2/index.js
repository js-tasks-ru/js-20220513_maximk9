...
export default class ProductForm {
  ...
  async render() {
    ...
    this.imageSortableList = new SortableList(
      { items: this.product.images.map(image => this.imageListElement(image)) }
    );
  ...
  uploadHandler = async (event) => {
    ...
    if (response && response.success) {
      ...
      this.imageSortableList.addItem(this.imageListElement(newImage));
    }
  }
  ...
  async fetchProduct(productId) {
    ...
    const productResponse = await fetchJson(this.productUrl);
